...
- 搜索支持歌手、专辑，可打开歌手详情查看歌曲和专辑。
- 桌面歌词改为接近网易云的单行条：悬停显示工具栏，支持卡拉 OK 填色、锁定与拖动。
- 桌面歌词每次打开都在桌面顶部水平居中，不记住上次拖动位置。
- 桌面歌词贴到屏幕顶部时，工具栏会立刻移到歌词下方，避免挡住继续往上拖。

### 修复

- 修复桌面歌词窗口贴顶后工具栏仍停在歌词上方的问题。
- 修复若干搜索、评论和歌词窗口的显示与稳定性问题。

## [3.0.10] - 2026-09-15

### 新增

- 当前播放歌曲会在列表左侧和歌单封面显示三条节奏条，悬停可暂停或继续播放。
- 选择皮肤弹窗的毛玻璃改为左侧栏、右侧栏分别设置。

### 修复

- 修复网易云歌单、每日推荐加载失败的问题。
- 修复双击网易云每日推荐歌曲时出现 “An object could not be cloned” 无法播放的问题。
- 修复播放队列按钮同时出现两处「播放队列」提示的问题。
- 修复在线列表和歌单详情不显示当前播放歌曲的问题。

## [3.0.9] - 2026-09-14

### 新增

- 播放栏进度条新增六种样式，默认使用全宽右进度条。
- 选择皮肤弹窗可自定义左侧栏透明度、右侧栏透明度和毛玻璃强度。
- 基本设置中的「字体设置」可同时选择字体和字号。

### 修复

- 从搜索结果进入歌单后，点返回会回到原来的搜索结果，而不是搜索首页。
- 全宽进度条不再重复显示时间。
- 播放按钮中的三角图标更居中。
- 窗口最小化、最大化、关闭按钮改为圆角矩形，更接近侧栏选中样式。
...
- Added a dedicated QQ Music settings page with login-based Cookie capture.
- Added QQ Music personalized recommendation entry and recommendation categories.

### Changed

- Rebranded this fork as LX Flow Music with isolated app data, protocol, and Windows application identity.
- Added GitHub Releases-based in-app updates for the Windows installer edition.
