...
<p align="center">基于 LX Music Desktop 的个人使用修改版，专注于 QQ 音乐与网易云音乐的账号推荐和歌单联动。</p>
...
- 新增 QQ 音乐账号视图：主页推荐、我的歌单、雷达推荐、推荐歌单、推荐新歌；“我的歌单”可浏览账号创建和收藏的歌单并在应用内播放。
- 新增独立的 QQ 音乐设置页：可通过登录窗口获取登录状态，不必手动粘贴 Cookie；登录信息只保存于本机应用数据中。
- 新增本地歌单手动同步：在“我的列表”右键选择“同步到 QQ音乐”，预览匹配结果后可新建 QQ 音乐歌单；不会覆盖或删除已有歌单。
