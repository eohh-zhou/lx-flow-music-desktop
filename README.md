...
<p align="center">基于 LX Music Desktop 的个人使用修改版，专注于企鹅音乐与网易云音乐的账号推荐和歌单联动。</p>
...
- 新增企鹅音乐每日推荐入口：在左侧导航中可直接打开，不需要手动进入企鹅音乐的深层歌单页面。
- 新增企鹅音乐账号视图：主页推荐、我的歌单、雷达推荐、推荐歌单、推荐新歌；“我的歌单”可浏览账号创建和收藏的歌单并在应用内播放。
- 新增独立的企鹅音乐设置页：可通过登录窗口获取登录状态，不必手动粘贴 Cookie；登录信息只保存于本机应用数据中。
- 新增本地歌单手动同步：在“我的列表”右键选择“同步到企鹅音乐”，预览匹配结果后可新建企鹅音乐歌单；不会覆盖或删除已有歌单。
- 新增网易云音乐推荐、账号歌单与歌单同步：登录后可浏览账号创建和收藏的歌单，查看每日推荐、私人 FM、推荐歌单和新歌，并可将本地歌单匹配后增量同步到网易云音乐；保留现有歌曲并按本地顺序排列。
- 推荐依据使用企鹅音乐账号的播放记录和歌单等数据，结合企鹅音乐的推荐结果。
...
| 包类型 | 文件名示例 | 适用场景 |
| --- | --- | --- |
| Setup 安装版 | `LXFlowMusic-v2.12.9-x64-Setup.exe` | 推荐。安装后可在软件内完成后续更新。 |
| 绿色免安装版 | `LXFlowMusic-v2.12.9-win_x64-green.7z` | 解压后直接运行 `LXFlowMusic.exe`。不支持自动安装更新。 |
