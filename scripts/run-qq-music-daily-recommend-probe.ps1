...
$probePath = Join-Path $PSScriptRoot 'qq-music-daily-recommend-probe.mjs'
...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($cookie)) { throw 'Cookie cannot be empty.' }
  $env:QQ_MUSIC_COOKIE = $cookie
  & $resolvedNodePath $probePath
  exit $LASTEXITCODE
} finally {
