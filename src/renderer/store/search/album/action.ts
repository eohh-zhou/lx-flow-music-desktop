...
const handleSortList = (list: AlbumInfoItem[], keyword: string) => {
  let arr: any[] = []
...
const setLists = (results: SearchResult[], page: number, text: string): AlbumInfoItem[] => {
  let totals = []
  let limit = 0
  let list = []
...
const setList = (datas: SearchResult, page: number, text: string): AlbumInfoItem[] => {
...
export const search = async(text: string, page: number, sourceId: LX.OnlineSource | 'all'): Promise<AlbumInfoItem[]> => {
  ...
  if (sourceId == 'all') {
    ...
    for (const source of sources) {
      ...
      task.push((music[source]?.album.search(text, page, listInfos.all.limit) ?? Promise.reject(new Error('source not found: ' + source))).catch((error: any) => {
        console.log(error)
        return {
          list: [],
          total: 0,
