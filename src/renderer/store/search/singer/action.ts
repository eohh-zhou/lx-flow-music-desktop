...
import type { SingerInfoItem } from './state'
...
const handleSortList = (list: SingerInfoItem[], keyword: string) => {
  let arr: any[] = []
...
const setLists = (results: SearchResult[], page: number, text: string): SingerInfoItem[] => {
  let totals = []
  let limit = 0
  let list = []
...
const setList = (datas: SearchResult, page: number, text: string): SingerInfoItem[] => {
...
export const search = async(text: string, page: number, sourceId: LX.OnlineSource | 'all'): Promise<SingerInfoItem[]> => {
  ...
  if (sourceId == 'all') {
    ...
    for (const source of sources) {
      ...
      task.push((music[source]?.singer.search(text, page, listInfos.all.limit) ?? Promise.reject(new Error('source not found: ' + source))).catch((error: any) => {
        console.log(error)
        return {
          list: [],
          total: 0,
    ...
    listInfo.noItemLabel = window.i18n.t('list__loading')
    listInfo.key = key
    return (music[sourceId]?.singer.search(text, page, listInfo.limit).then((data: SearchResult) => {
      if (key != listInfo.key) return []
