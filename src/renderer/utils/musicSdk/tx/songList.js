...
export default {
  ...
  async getDailyRecommend() {
    const result = await getQQMusicDailyRecommend()
    return {
      list: result.list,
      page: 1,
      limit: result.list.length || 30,
  ...
  async getRadarRecommend(id) {
    const result = await getQQMusicRadarTracks(Number(id))
    return {
      list: result.list,
      page: 1,
      limit: result.list.length || 30,
  ...
  async getNewSongRecommend(type) {
    const result = await getQQMusicNewSongs(Number(type))
    return {
      list: result.list,
      page: 1,
      limit: result.list.length || 30,
