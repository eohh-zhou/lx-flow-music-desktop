  ...
  >
    <main class="scroll" :class="$style.main">
      <header :class="$style.header">
        <h2>{{ $t('netease_music_sync_title') }}</h2>
        <p>{{ listName }}</p>
      </header>

      <section :class="$style.content">
        <p v-if="status == 'loading'" :class="$style.status">{{ $t('netease_music_sync_matching') }}</p>
        <p v-else-if="status == 'syncing'" :class="$style.status">{{ $t('netease_music_sync_writing') }}</p>
        <p v-else-if="status == 'notConfigured'" :class="$style.status">{{ $t('netease_music_sync_login_required') }}</p>
        <p v-else-if="status == 'empty'" :class="$style.status">{{ $t('netease_music_sync_empty') }}</p>

        <template v-if="preview">
          <dl :class="$style.summary">
            <div>
              <dt>{{ $t('netease_music_sync_total') }}</dt>
              <dd>{{ preview.total }}</dd>
            </div>
            <div>
              <dt>{{ $t('netease_music_sync_matched') }}</dt>
              <dd>{{ preview.matched }}</dd>
            </div>
            <div>
              <dt>{{ $t('netease_music_sync_unmatched') }}</dt>
              <dd>{{ preview.unmatched.length }}</dd>
            </div>
            <div>
              <dt>{{ $t('netease_music_sync_duplicates') }}</dt>
              <dd>{{ preview.duplicates }}</dd>
            </div>
          </dl>

          <div v-if="preview.unmatched.length" :class="$style.unmatched">
            <h3>{{ $t('netease_music_sync_unmatched_title') }}</h3>
            <ul class="scroll">
        ...
        <p v-if="status == 'error'" :class="[$style.result, $style.error]">
          {{ $t('netease_music_sync_failed') }}<br>
...
import {
  commitNeteaseMusicPlaylistSync,
  getNeteaseMusicStatus,
  previewNeteaseMusicPlaylistSync,
} from '@renderer/utils/ipc'

export default {
  name: 'NeteaseMusicSyncModal',
  props: {
    visible: {
      type: Boolean,
      default: false,
  ...
  setup(props, { emit }) {
    ...
    const toSyncTrack = musicInfo => ({
      ...
      neteaseSongId: musicInfo.source == 'wy' ? String(musicInfo.meta?.songId ?? '') : undefined,
    })
    ...
    const loadPreview = async() => {
      ...
      try {
        const neteaseMusicStatus = await getNeteaseMusicStatus()
        if (currentLoadId != loadId) return
        if (!neteaseMusicStatus.configured) {
        ...
        const nextPreview = await previewNeteaseMusicPlaylistSync({
          name: listName.value,
    ...
    const commitSync = async() => {
      ...
      try {
        result.value = await commitNeteaseMusicPlaylistSync({ token: preview.value.token })
    ...
    const openSettings = () => {
      closeModal()
      void router.push({ path: '/setting', query: { name: 'SettingNeteaseMusic' } })
    }
